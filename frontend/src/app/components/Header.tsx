          ...
          <div className="hidden md:flex items-center space-x-4">
            {loading ? (
              ...
              <div className="flex items-center space-x-4">
                <Link href={`/user/${user.nickname}`} className="flex items-center space-x-2 text-blue-900 font-serif hover:text-blue-700 transition-colors">
                  <User className="h-5 w-5" />
                  <span>{user.first_name}</span>
                </Link>
                <button
                  onClick={logout}
                  className="cursor-pointer flex items-center space-x-2 px-4 py-2 text-red-600 hover:text-red-800 transition-colors font-serif"
                >
          ...
          <div className="md:hidden border-t border-blue-100 py-4">
            <div className="flex flex-col space-y-3">
              {loading ? (
                ...
                <>
                  <Link href={`/user/${user.nickname}`} className="flex items-center space-x-2 px-4 py-2 text-blue-900 font-serif hover:text-blue-700 transition-colors">
                    <User className="h-5 w-5" />
                    <span>{user.first_name}</span>
                  </Link>
                  <button
                    onClick={logout}
                    className="flex items-center space-x-2 px-4 py-2 text-red-600 hover:text-red-800 transition-colors font-serif"
                  >
                    <LogOut className="h-4 w-4" />
                    <span>Sair</span>
                  </button>
